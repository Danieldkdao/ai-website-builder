...
readFiles({
  "files": ["package.json", "app/page.tsx", "components/Button.tsx"]
})

5. Reading files to understand the codebase:
readFiles({
  "files": ["tsconfig.json", "next.config.js", "app/layout.tsx"]
})
...
7. Running multiple terminal commands (one at a time):
terminal({
...
❌ print(createOrUpdateFiles(files=[...]))
❌ print(readFiles(files=["package.json"]))
❌ default_api.createOrUpdateFiles(files=[...])
❌ default_api.readFiles(files=[...])
❌ console.log(createOrUpdateFiles({files: [...]}))
...
- No print() or console.log() wrappers
- No API prefixes (default_api., api., tools.)
- No special class constructors
- Call tools directly with their exact names
